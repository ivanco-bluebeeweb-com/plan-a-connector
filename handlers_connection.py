"""Connection lifecycle for Plan A Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from plan_a_client import PlanAClient
...
@chat.function(
    "connect_plan_a",
    "Connect Plan A account via credentials.",
    action_type="write",
    chain_callable=True,
    event="plan-a-connector.connect_plan_a",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_plan_a(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Plan A Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Plan A Account",
...
@chat.function(
    "list_connections",
    "List connected Plan A accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_plan_a",
    "Disconnect Plan A account.",
    action_type="write",
    chain_callable=True,
    event="plan-a-connector.disconnect_plan_a",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_plan_a(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
